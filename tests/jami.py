        ...
        if(self.expectedUpdate):
            if(isinstance(packet, RoutingUpdate)):
                ...
                if not self.expectedUpdate[testCaseNum].has_key(self.num_rx):
                    ...
                    print "FAILED: Too many items in packet: "+str(packet.paths)+", should be "+str(self.expectedUpdate[testCaseNum][self.num_rx])
                    failed = True
                elif len(packet.paths) < len(self.expectedUpdate[testCaseNum][self.num_rx]):
                    print "FAILED: Too few items in packet: "+str(packet.paths)+", should be "+str(self.expectedUpdate[testCaseNum][self.num_rx])
                    failed = True
                else:
                    for dest,cost in packet.paths.iteritems():
                        if dest not in self.expectedUpdate[testCaseNum][self.num_rx].keys():
                            ...
                            print "FAILED: cost "+str(cost)+" for destination "+str(dest)+" incorrect, should be "+str(self.expectedUpdate[testCaseNum][self.num_rx][dest])
                            failed = True
